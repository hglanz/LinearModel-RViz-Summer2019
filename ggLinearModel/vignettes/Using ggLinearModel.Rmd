...
### Interactive mode
Setting the interactive parameter to TRUE will activate a feature that allows you to view data about any point on the graph when you hover over it, and the will give the equation of the lines on the graph when hovered over.
```{r}
#ggLinearModel(mtcars, wt, mpg, cyl, interactive = T)
```

### Plotly mode
Making the plotly parameter equal TRUE will turn the graph into a plotly graph instead of ggplot.
```{r}
#ggLinearModel(iris, Petal.Width, Petal.Length, Species, plotly = T)
```
...
```{r}
#ggLinearModel(mtcars, wt, mpg, cyl, title = "The Affect of a car's weight on the MPG", xlabel = 'Weight (lbs)', ylabel = 'Miles per Gallon', legendTitle = 'Number of Cylinders in engine')
```
